...
ISR:
    BTFSC ((INTCON) and 07Fh), 2 ;Es la interrupcion de TMR0?
    CALL INT_TMR0
    BTFSC ((PIR1) and 07Fh), 0 ;Es la interrupcion de TMR1?
    CALL INT_TMR1
    BTFSC ((PIR1) and 07Fh), 1 ;Es la interrupcion de TMR2?
    CALL INT_TMR2
    BTFSC ((INTCON) and 07Fh), 0 ;Es la interrupción del puerto B?
    CALL INT_IOCB
POP:
    BANKSEL STATUS_TEMP
    SWAPF STATUS_TEMP, W
...
HORA:
    ...
    BTFSC alarma_hora, 0 ; Evaluo si la alarma de la hora está sonando para la función del puerto ((PORTB) and 07Fh), 4
    CALL VER_PUERTO_RB4
...
ESTADO1_TIMER:
    ...
    BTFSS PORTB, 3
    BSF estado_timer, 0
    BTFSC alarma_hora, 0 ; ((PORTB) and 07Fh), 4 tiene distinta funcionalidad si estamos con la alarma sonando
    GOTO ESTA_SONANDO
    BTFSC alarma_timer, 0
    GOTO ESTA_SONANDO2
    BTFSS PORTB, 4 ;activamos el timer si presionamos el ((PORTB) and 07Fh), 4
    INCF activar_timer
    BCF ((INTCON) and 07Fh), 0
    RETURN
ESTADO2_TIMER:
...
SEGUNDOS_TIMER_255:
    ...
    MOVWF segundos_timer
    RETURN
; ----------------- código que servirá para la función del puerto ((PORTB) and 07Fh), 4 si la alarma está prendida
VER_PUERTO_RB4:
    BTFSS PORTB, 4
    CALL APAGAR_ALARMA_HORA
    RETURN
